...
class AutoTraderV5:
    ...
    def generate_signal(self, market: Dict, price: float, no_price: float = None) -> Optional[Dict]:
        ...
        if direction:
            ...
            # 3️⃣ 15分钟UT Bot趋势锁（带强动能穿透功能）
            # 🚨 多重豁免权：核弹级信号 OR 1H大势+极强动能
            if not is_whale_attack:  # 非核弹模式下才检查15m趋势
                if ut_hull_trend != 'NEUTRAL':
                    # ==========================================
                    # 🔥 强动能穿透逻辑 (MTF Divergence战术)
                    # ==========================================
                    # 当1H大势支持，且当前动能极强时，说明正在发生"强势反转"
                    # 此时15m指标因为滞后性还未反应，应该强行穿透！
                    if direction == 'LONG' and ut_hull_trend == 'SHORT':
                        # 做多遇15m SHORT：检查是否符合"完美抄底反转"条件
                        if trend_1h == 'LONG' and score >= 4.5:
                            print(f"🔥 [UT豁免] 1H大势=LONG且动能极强(score={score:.2f})！无视15m滞后阻力，强制抄底反转！")
                        else:
                            print(f"🛑 [15m UT Bot锁] 拒绝做多！15m趋势=SHORT与方向不符")
                            return None
                    elif direction == 'SHORT' and ut_hull_trend == 'LONG':
                        # 做空遇15m LONG：检查是否符合"完美摸顶反转"条件
                        if trend_1h == 'SHORT' and score <= -4.5:
                            print(f"🔥 [UT豁免] 1H大势=SHORT且动能极强(score={score:.2f})！无视15m滞后阻力，强制摸顶反转！")
                        else:
