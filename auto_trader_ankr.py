...
def main():
    ...
    def learning_report():
        """学习报告路由"""
        try:
            from generate_learning_report import (
                get_overall_stats, get_oracle_accuracy, get_threshold_search
            )
            ...
            # 生成HTML（复用generate_learning_report的逻辑）
            from generate_learning_report import generate_html_report
    ...
    def refresh_report():
        ...
        try:
            ...
            if not os.path.exists(prediction_db):
                return '{"success": false, "error": "数据库不存在"}', 404
            ...
            return '{"success": true, "message": "报告已刷新"}'
        except Exception as e:
    ...
    def start_flask():
        ...
        try:
            # 使用不同的端口避免冲突
            # Flask会自动绑定到主程序使用的端口（通过环境变量）
            port = int(os.getenv('PORT', 8080))
            print(f"[FLASK] 学习报告API启动: http://0.0.0.0:{port}/learning/report")
