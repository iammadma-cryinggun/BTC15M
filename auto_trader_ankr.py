...
CONFIG = {
    ...
    'risk': {
        ...
        'max_stop_loss_pct': 0.20,      # 最大止损20%（安全第一）
    },
            ...
            if is_stop_loss:
                ...
                if entry_price is None:
                    ...
                    if best_bid and best_bid >= min_acceptable_sl:
                        ...
                        # 🚨 熔断检查：如果best_bid已经远低于止损线（>40%），说明流动性枯竭
                        if best_bid < entry_price * 0.60:
                            print(f"       [熔断机制] 🚨 流动性真空！best_bid({best_bid:.4f}) < 止损线60%")
                            print(f"       [熔断机制] 入场价={entry_price:.4f}, 止损线={entry_price*0.8:.4f}")
                            print(f"       [熔断机制] 🛡️ 拒绝血亏砸盘，改挂防守限价单 @ {entry_price*0.80:.4f}")
                            # 宁可归零博反弹，也不以极端低价贱卖
                            close_price = entry_price * 0.80
                            use_limit_order = True
                        else:
                            # 情况2：best_bid太低但未触发熔断，用砸盘价确保成交
                            # 🔥 关键修复：止损必须保命，用best_bid-0.05主动砸盘
                            # 即使滑点5%也比亏100%好！
                            close_price = max(0.01, best_bid - 0.05)
                            use_limit_order = False  # ✅ 改为市价单，立即成交
                            print(f"       [止损模式] ⚡ best_bid太低({best_bid:.4f})，砸盘价 @ {close_price:.4f} (滑点保护)")
                    else:
                        # 情况3：无法获取best_bid，用入场价90%砸盘保命
                        # 🔥 宁可接受10%滑点，也不能挂单等待导致归零
                        close_price = max(0.01, entry_price * 0.90)
                        use_limit_order = False
                        print(f"       [止损模式] ⚡ 无best_bid，砸盘价 @ {close_price:.4f} (保守滑点10%)")
