...
# CVD滚动窗口（秒）
# 🚀 走火入魔版：从 120 秒直接压缩到 10 秒！只看当下的"火星子"！
# 理由：用"预期抢跑"打败机构延迟 - 不等成交确认，只看盘口失衡意图
CVD_WINDOW_SEC = 10
...
class BinanceOracle:
    ...
    def _calc_signal_score(self) -> float:
        """
        🚀 终极前瞻版：纯盘口失衡预期 + 毫秒级点火跟车

        核心理念：不等成交确认，只看挂单意图（抢跑）
        - 机构拉盘前必先挂巨量买单托盘
        - 我们在机构撤单前抢先上车
        """
        score = 0.0

        # 1. 极速微观 CVD (10秒窗口，4万刀净流入即满分)
        cvd_score = max(-5.0, min(5.0, self.cvd / 40000.0))
        score += cvd_score
        ...
        # 绝杀 A：纯盘口压制 (不需要成交确认)
        # 如果一方挂单是另一方的 5 倍以上 (|imbalance| > 0.6)，说明主力兵力极度悬殊
        # 此时价格还没动，做市商还没撤单，我们直接抢跑！
        if imbalance > 0.75:
            print(f"       [🚀 PRE-ARBITRARY] 下方托盘如山 (imbalance={imbalance:.2f})，抢在价格变动前做多！")
            return 10.0   # 下方托盘如山，价格必涨，抢先买 YES！
        elif imbalance < -0.75:
            print(f"       [☄️ PRE-ARBITRARY] 上方压盘如山 (imbalance={imbalance:.2f})，抢在价格变动前做空！")
            return -10.0  # 上方压盘如山，价格必跌，抢先买 NO！

        # 绝杀 B：十秒极速爆破 (不管盘口，瞬间吃货直接跟)
        # 只要 10 秒内，瞬间爆发超过 8 万美金的单向吃单，毫不犹豫跟车！
        if self.cvd > 80000:
            print(f"       [🚀 10S EXPLOSION] 10秒爆买 {self.cvd/1000:.1f}K USD，极速跟车做多！")
            return 10.0
        elif self.cvd < -80000:
            print(f"       [☄️ 10S EXPLOSION] 10秒爆卖 {abs(self.cvd)/1000:.1f}K USD，极速跟车做空！")
            return -10.0
