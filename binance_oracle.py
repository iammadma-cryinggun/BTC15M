...
# CVD滚动窗口（秒）
# 🚀 爆改：从 900 (15分钟) 缩短到 120 (2分钟)，只抓当下的极速爆发！
CVD_WINDOW_SEC = 120
...
class BinanceOracle:
    ...
    def _calc_signal_score(self) -> float:
        ...
        # 1. 极速 CVD 分（因为窗口缩短到2分钟了，阈值同步降低，每 10万USD 净流入得1分）
        cvd_score = max(-5.0, min(5.0, self.cvd / 100000.0))
        score += cvd_score
        ...
        # 💥 3. 核弹级失衡直接熔断抢跑 (Whale Override)
        # 逻辑：如果某一方挂单墙资金是另一方的 4 倍以上 (|imbalance| > 0.6)，且当前有顺向资金吃单
        if imbalance > 0.6 and cvd_score > 0:
            return 10.0   # 满分做多！无视其他，直接上车
        elif imbalance < -0.6 and cvd_score < 0:
            return -10.0  # 满分做空！无视其他，直接砸盘
